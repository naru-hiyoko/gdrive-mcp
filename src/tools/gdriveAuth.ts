...
export const gdrive_get_auth_url_tool: Tool = {
  name: 'gdrive_get_auth_url',
  description:
    'Google Drive の OAuth2 認証 URL を取得します。ブラウザでこの URL を開いて Google アカウントでログイン・許可し、' +
...
export async function gdriveGetAuthUrl(): Promise<ToolResult> {
  try {
      ...
      content: [
        {
          type: 'text',
          text:
            `以下の URL をブラウザで開いて Google アカウントで認証してください:\n\n${url}\n\n` +
...
export const gdrive_complete_auth_tool: Tool = {
  name: 'gdrive_complete_auth',
  description:
    '認証コードまたはリダイレクト URL を受け取り、Google Drive の token.json を保存します。' +
  ...
  inputSchema: {
    ...
    properties: {
      code_or_redirect_url: {
        type: 'string',
        description:
          '認証後にリダイレクトされた URL（例: http://localhost:...?code=...）または認証コード文字列。',
      },
    },
...
export async function gdriveCompleteAuth(args: unknown): Promise<ToolResult> {
  try {
    const { code_or_redirect_url } = args as { code_or_redirect_url: string }
