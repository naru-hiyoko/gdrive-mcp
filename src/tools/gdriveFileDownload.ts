...
import type { GdriveDownloadResult, ToolResult } from '../types/index.js'
...
export const gdrive_file_download_tool: Tool = {
  ...
  inputSchema: {
    ...
    properties: {
      file_id: {
        type: 'string',
        description:
          'ダウンロードするファイルの ID。Google Drive の URL から取得できます: ' +
          'https://docs.google.com/spreadsheets/d/<FILE_ID>/edit',
      },
      output_path: {
        type: 'string',
        description:
          '保存先のファイルパス（省略時はカレントディレクトリにファイル名で保存）。' +
          '例: /tmp/output.csv',
      },
      sheet_name: {
        type: 'string',
        description:
          'スプレッドシートの特定シートを指定する場合のシート名。省略時は先頭シートをエクスポートします。',
      },
    },
    required: ['file_id'],
  },
...
function downloadUrl(url: string, accessToken: string, dest: string): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    const req = https.get(options, (res) => {
      if (res.statusCode && [301, 302, 307, 308].includes(res.statusCode)) {
        ...
        if (!location) return reject(new Error('リダイレクト先 URL が見つかりません'))
...
function downloadUrlNoAuth(url: string, dest: string): Promise<void> {
  return new Promise((resolve, reject) => {
    https
      .get(url, (res) => {
        if (res.statusCode && [301, 302, 307, 308].includes(res.statusCode)) {
...
export async function gdriveFileDownload(args: unknown): Promise<ToolResult> {
  try {
    const { file_id, output_path, sheet_name } = args as {
      file_id: string
      output_path?: string
      sheet_name?: string
    }
    ...
    if (sheet_name && mimeType === 'application/vnd.google-apps.spreadsheet') {
      ...
      const res = await drive.files.export(
        { fileId: file_id, mimeType: exportInfo.mime },
        { responseType: 'stream' }
      )
